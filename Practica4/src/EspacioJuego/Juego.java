...
public class Juego {
    ...
    public String jugar(double n){
    	double j1 = 0.0;
    	double em = 0.0;
    	double j2 = 0.0;
    	for(int i = 0; i < n; i++){
    		if(jugarPartida(false)==1){
    			j1++;
    		}else if(jugarPartida(false)==0){
    			em++;
    		}else{
    			j2++;
    		}
    	...
    	return "En "+ n + " partidas, el J1 ha ganado " + (j1/n)*100 + "%, han empatado: " + (em/n)*100 +"% y el J2 ha ganado: " + (j2/n)*100 + "%";
    }
